...
- Confirm `schedule.mode` is `INTERVAL`: the backpressure gate is released by a snapshot draining the pending set. Under `MANUAL` and `AFTER_AUTOSAVE` (which currently behaves like `MANUAL`) nothing drains it unless you create a snapshot, so the baseline just waits. A clean shutdown does take a final snapshot, so in those modes the baseline advances at most `dirtyHighWaterMark` chunks per session - and a session ended by kill -9 or a panel force-stop makes no net progress at all
- Rehearse a restore before you actually need one (the offline CLI's `verify` / `restore` do not need a running server)

Two boundaries worth stating plainly. First, the gate covers the baseline full scan only; the degraded-window backfill is not bounded by `dirtyHighWaterMark` (it keeps no per-region progress, so pausing it would permanently lose the regions it had not reached), and while it runs the memory ceiling above does not hold. Second, backpressure bounds BetterBackup's own memory growth; it is not a substitute for sizing the JVM correctly. When `-Xms` plus `-XX:+AlwaysPreTouch` commits a heap close to physical memory, any background task can push the process into the kernel OOM killer, and ZGC's heap multi-mapping inflates the RSS accounting further. On the machine in #5, BetterBackup's own growth was on the order of one percent of the 48 GB heap.
