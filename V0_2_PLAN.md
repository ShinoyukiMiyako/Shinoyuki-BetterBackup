...
| 字段 | 值 |
|---|---|
| 状态 | 已批准 (2026-06-10); 主线二 (离线部分回退) 与原划 v0.3 的在线单 chunk 回退已提前落地并入 main (见 §二 / §六 / §五), 其余主线待 v0.1.0 发版后开工 |
...
状态: 已实现 (commit 60fafc8 "feat(cli): 单 chunk / 区域 离线部分回退 (PLAN v0.2 主线二)", 经 merge 213a36d 并入 main)。下述范围为原始设计, 均已落地。
...
5. 在线版 (要求 chunk unloaded + 通知 BAS 状态机) 原推 v0.3, 现已提前实现 (见 §六 / commit 83ec2f9)
...
进度订正: 主线二 (离线部分回退, commit 60fafc8) 与原划 v0.3 的在线单 chunk 回退
(commit 83ec2f9) 已在 v0.1.0 发版前提前落地并入 main (merge 213a36d)。剩余排期收敛为:
v0.1.0 发版 -> 主线一 (NBT dedup B) -> 清账 1/2/3 -> 主线三 (manifest 差分)
-> 清账其余 -> v0.2.0 发版。
...
在线单 chunk 恢复: 原划 v0.3, 现已提前实现 (commit 83ec2f9 "feat(command): 在线区块
回退命令 restore-chunk-live", 经 merge 213a36d 并入 main), 不再属于"不在 v0.2"范畴;
NeoForge 1.21.1 移植 (独立 track, 先做 1.21.x save 路径侦察); zstd / blake3 (无生产
需求佐证, 搁置); Web UI / 远程上传 (永不)。
