...
/**
 * Phase 5: BetterBackup Prometheus metrics HTTP exporter.
 *
 * <p>用 JDK 内置 {@link HttpServer} 在 {@code bindAddress:port/metrics} 端点
 * 暴露 {@link BetterBackupMetrics} 当前快照 (Prometheus exposition format).
 * 零外部依赖, 不引入 simpleclient 库.
 *
 * <p><b>线程模型</b>: 单 daemon 线程 executor. Prometheus 默认 scrape 间隔 15s,
 * 单 mod 单 endpoint 没必要多线程. metrics.snapshot() 内部用
 * {@code LongAdder.sum() / AtomicLong.get()} 全部 lock-free, 不阻塞 BetterBackup
 * worker 主路径.
 *
 * <p><b>启动失败</b>: bind 端口被占用 / 地址非法 -> {@link #start()} 抛 IOException.
 * 调用方 (BetterBackupMod onServerStarting) 应捕获并 log error, 服务端继续运行
 * 不 crash. 跟 BetterBackup degraded mode 哲学一致.
 *
 * <p><b>请求异常</b>: handler 内部所有异常 try-catch + log, 不 propagate 到
 * HttpServer (避免单次抓取异常 kill 整个 server 线程). 失败的抓取 Prometheus
 * 自身会重试.
 */
public final class PrometheusExporter {
    ...
    public PrometheusExporter(BetterBackupMetrics metrics, String bindAddress, int port) {
    ...
    public synchronized void start() throws IOException {
        ...
        s.setExecutor(Executors.newSingleThreadExecutor(r -> {
            Thread t = new Thread(r, "BetterBackup-Prometheus-" + THREAD_INDEX.incrementAndGet());
            t.setDaemon(true);
            return t;
        }));
        s.start();
        this.server = s;
        this.started = true;
        LOGGER.info("[BetterBackup] Prometheus exporter listening on {}:{}{}",
    ...
    public synchronized void stop() {
        ...
        try {
            ...
            LOGGER.warn("[BetterBackup] Prometheus exporter stop threw", t);
        }
        this.server = null;
        this.started = false;
        LOGGER.info("[BetterBackup] Prometheus exporter stopped");
    }
    ...
    private void handleMetrics(HttpExchange exchange) {
        try (exchange) {
            ...
            LOGGER.error("[BetterBackup] Prometheus /metrics handler threw", t);
        }
    }
    ...
    private void handleNotFound(HttpExchange exchange) {
        try (exchange) {
            ...
            LOGGER.error("[BetterBackup] Prometheus 404 handler threw", t);
        }
    }
}
